...
/**
 *
 * Draws the EquitySwapsPanel
 */
public class EquitySwapsPanel extends JPanel {
    private Integer n_clusters;
    private Integer n_epochs;
    private List<Ellipse2D> mylist;
    private KmeansForEquities kfes;
    //List pointList;
    //Color selectedColor;
     ...
     */
    public EquitySwapsPanel(String aclusters,String aepochs) {
        n_clusters=Integer.valueOf(aclusters);
        n_epochs=Integer.valueOf(aepochs);
        mylist=new ArrayList<>();
        kfes=new KmeansForEquities();
        
        setBackground(new Color(250,250,250));
        
        Ellipse2D mipunto;
        EquitySwapsdata data=new EquitySwapsdata();
        for (int i=0;i<EquitySwapsdata.listequityswaps.size();i++) {
            data=EquitySwapsdata.listequityswaps.get(i);
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.drawString("A-Transaction Code", 100, 420);
        //b-point
        g2.drawLine(14,  0, 14,600);
        g2.drawString("B-Equity Swap", 20, 100);
        //c-point
        g2.drawLine(14,440,500,114);
        g2.drawString("C-Records", 300, 200);
    ...
    private class KmeansForEquities extends Kmeans {
        
        public KmeansForEquities() {
        }
        public void Execute() {
            ...
            for(int j = 0; j < Kmeans.Cluster.length; j++) {
                ...
                int k=rn.nextInt(0, EquitySwapsdata.listequityswaps.size());
                EquitySwapsdata tdata=EquitySwapsdata.listequityswaps.get(k);
        ...
        */
        public final void DataInitialization() {
            ...
            EquitySwapsdata esdata=new EquitySwapsdata();
            for (int i=0;i<EquitySwapsdata.listequityswaps.size();i++) {
                esdata=EquitySwapsdata.listequityswaps.get(i);
                
                midato=new Datasdata(esdata);
