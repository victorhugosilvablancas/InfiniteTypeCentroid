...
public class SharesPanel extends JPanel {
     ...
     */
    public SharesPanel(String aclusters,String aepochs) {
        n_clusters=Integer.valueOf(aclusters);
        n_epochs=Integer.valueOf(aepochs);
        mylist=new ArrayList<>();
        kfs=new KmeansForShares();
        
        setBackground(new Color(250,250,250));
        
        Ellipse2D mipunto;
        Sharesdata data=new Sharesdata();
        for (int i=0;i<Sharesdata.listshares.size();i++) {
            data=Sharesdata.listshares.get(i);
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.drawString("A-Transaction Shares", 100, 420);
        //b-point
    ...
    private class KmeansForShares extends Kmeans {
        
        public KmeansForShares() {
        }
        public void Execute() {
            ...
            for(int j = 0; j < Kmeans.Cluster.length; j++) {
                ...
                int k=rn.nextInt(0, Sharesdata.listshares.size());
                Sharesdata tdata=Sharesdata.listshares.get(k);
        ...
        */
        public final void DataInitialization() {
            ...
            Sharesdata sdata=new Sharesdata();
            for (int i=0;i<Sharesdata.listshares.size();i++) {
                sdata=Sharesdata.listshares.get(i);
                
                midato=new Datasdata(sdata);
