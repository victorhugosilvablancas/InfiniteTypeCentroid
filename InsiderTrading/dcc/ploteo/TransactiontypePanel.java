...
/**
 *
 * Draws the TransactiontypePanel
 */
public class TransactiontypePanel extends JPanel {
    ...
    private KmeansForTransactions kft;
    //List pointList;
    //Color selectedColor;
     ...
     */
    public TransactiontypePanel(String aclusters,String aepochs) {
        n_clusters=Integer.valueOf(aclusters);
        n_epochs=Integer.valueOf(aepochs);
        mylist=new ArrayList<>();
        kft=new KmeansForTransactions();
        
        setBackground(new Color(250,250,250));
        
        Ellipse2D mipunto;
        Transactionstypedata data=new Transactionstypedata();
        for (int i=0;i<Transactionstypedata.listtransactions.size();i++) {
            data=Transactionstypedata.listtransactions.get(i);
    ...
    protected void paintComponent(Graphics g) {
        ...
        g2.drawString("A-Records", 100, 420);
        //b-point
        g2.drawLine(14,  0, 14,600);
        g2.drawString("B-Transaction Date", 20, 100);
        //c-point
    ...
    private class KmeansForTransactions extends Kmeans {
        
        public KmeansForTransactions() {
        }
        public void Execute() {
            ...
            for(int j = 0; j < Kmeans.Cluster.length; j++) {
                ...
                int k=rn.nextInt(0, Transactionstypedata.listtransactions.size());
                Transactionstypedata tdata=Transactionstypedata.listtransactions.get(k);
        ...
        */
        public final void DataInitialization() {
            ...
            Transactionstypedata tdata=new Transactionstypedata();
            for (int i=0;i<Transactionstypedata.listtransactions.size();i++) {
                tdata=Transactionstypedata.listtransactions.get(i);
                
                midato=new Datasdata(tdata);
                Datasdata.Dataset.add(midato);
            }
        } //DataInitialization
        
    } //KmeansForTransactions
